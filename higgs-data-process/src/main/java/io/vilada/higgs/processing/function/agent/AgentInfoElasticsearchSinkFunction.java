 ...
 */
@Slf4j
public class AgentInfoElasticsearchSinkFunction implements ElasticsearchSinkFunction<List<AgentInfo>> {

    @Override
    public void process(List<AgentInfo> agentInfoList, RuntimeContext ctx, RequestIndexer indexer) {
        for (AgentInfo agentInfo : agentInfoList) {
            byte[] agentInfoArray;
            try {

                agentInfoArray = HiggsJobContext.getInstance().getJsonObjectMapper()
                        .writeValueAsBytes(agentInfo);
            } catch (Exception e) {
                log.error("RefinedSpanElasticsearchSinkFunction process failed, refinedSpan can not write to json.", e);
                continue;
            }

            indexer.add(Requests.indexRequest().index(AGENT_INFO_INDEX)
                    .type(AGENT_INFO_TYPE).source(agentInfoArray));
        }
    }
}
