 ...
 * @author mjolnir
 */
public class HttpServerSpanProcessor implements SpanProcessor {

    @Override
    public void processTransactionName(RefinedSpan refinedSpan) {
        String url = refinedSpan.getSpanTags().get(Tags.HTTP_URL.getKey());
        refinedSpan.getExtraContext().setSpanTransactionName(url);
    }
