...
@Service
class DatabaseElapsedTopNOutBOConverter extends DatabaseTopNOutBOConverter {
    ...
    public DatabaseTopNOutBO convert(ConverterInput input) {
        ...
        topNOutBO.setGroupedFieldName(input.getGroupedInput().getGroupedUnits().get(0).getGroupedField());
        topNOutBO.setStatsFieldName(input.getGroupedInput().getGroupedUnits().get(0).getAggrUnitList().get(0).getField());
            ...
            for (Aggregation aggr : aggregations.asList()) {
                if (aggr instanceof Terms) {
                    for (Terms.Bucket bucket : ((Terms) aggr).getBuckets()) {
                        ...
                        input1.setStats((InternalStats) bucket.getAggregations().asList().get(0));
                    ...
                    DatabaseTopNOutUnitConverter.ConverterInput input1 = new DatabaseTopNOutUnitConverter.ConverterInput();
                    input1.setGroupedFieldValue(null);
                    input1.setStats((InternalStats) aggr);
                    DatabaseTopNOutUnit overallUnit = topNOutUnitConverter.convert(input1);
