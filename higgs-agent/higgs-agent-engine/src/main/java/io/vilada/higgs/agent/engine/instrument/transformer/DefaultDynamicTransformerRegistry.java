...
public class DefaultDynamicTransformerRegistry implements DynamicTransformerRegistry {
    private static final Logger log = LoggerFactory.getLogger(DefaultDynamicTransformerRegistry.class);
    ...
    public RequestHandle onRetransformRequest(Class<?> target, final ClassFileTransformer transformer) {
        ...
        if (log.isInfoEnabled()) {
            log.info("added retransformer classLoader: {}, class: {}, registry size: {}", target.getClassLoader(), target.getName(), transformerMap.size());
        }
...
//        }
        ...
        if (log.isInfoEnabled()) {
            log.info("added dynamic transformer classLoader: {}, className: {}, registry size: {}", classLoader, targetClassName, transformerMap.size());
        }
    }
    ...
    private void add(TransformerKey key, ClassFileTransformer transformer) {
        ...
        if (prev != null) {
            throw new HiggsEngineException("Transformer already exists. TransformKey: " + key + ", transformer: " + prev);
        }
    }

    private TransformerKey createTransformKey(ClassLoader classLoader, String targetClassName) {
        final String classInternName = JavaBytecodeUtil.javaNameToJvmName(targetClassName);
    ...
    public ClassFileTransformer getTransformer(ClassLoader classLoader, String targetClassName) {
        ...
        if (transformer != null) {
            if (log.isInfoEnabled()) {
                log.info("removed dynamic transformer classLoader: {}, className: {}, registry size: {}", classLoader, targetClassName, transformerMap.size());
            }
        }
